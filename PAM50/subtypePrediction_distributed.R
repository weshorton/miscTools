...
if (!is.na(metaFile)){
  ...
  metaFile <- paste(inputDir, metaFile, sep="/")
  meta_df <- read.table(metaFile, header = T, stringsAsFactors = F)
  ## Get columns
  sampCol <- metaCols[1]
  treatCol <- metaCols[2]
  ## Get treats
  treats_v <- sort(unique(meta_df[[treatCol]]))
}

### Get colors
library(RColorBrewer)
pairs_v <- brewer.pal(12, "Paired")
...
pamout.centroids<-read.table(trainCentroids,sep="\t",header=T,row.names=1)
	...
	plotInfoName <- paste(inputDir, paste(short, "_heatmapColorLegend.txt", sep=""),sep="/")
	...
	# Create key
	subtypeKey <- data.frame("Class" = c("Basal", "Her2", "LumA", "LumB", "Normal"),
	                         "Color" = c("red", "hotpink", "darkblue", "skyblue", "green"))
	plotColors <- as.data.frame(cbind(subtypeColors, conf.colors))
	plotColors$Sample <- rownames(plotColors)
	plotInfo <- merge(plotColors, subtypeKey, by.x = "subtypeColors", by.y = "Color", sort = F)
	
	# Add treatment info
	if (!is.na(metaFile)){
	  ## Merge with treatment info
	  plotInfo <- as.data.table(merge(meta_df[,metaCols], plotInfo, by.x = sampCol, by.y = "Sample", sort = F))
	  ## Write
	  write.table(plotInfo, file = plotInfoName, sep = '\t', quote = F, row.names = F)
	  ## Update treat info into color info
	  for (i in 1:length(treats_v)){
	    currTreat_v <- treats_v[i]
	    plotInfo[get(treatCol) == currTreat_v, (treatCol) := pairs_v[i]]
	  } # for i
	  ## Fix formatting
	  plotArg <- as.matrix(plotInfo[,mget(c(treatCol, "subtypeColors", "conf.colors"))])
	  rownames(plotArg) <- plotInfo[[sampCol]]
	} else {
	  ## Write out
	  write.table(plotInfo, file = plotInfoName, sep = '\t', quote = F, row.names = F)
	  ## Fix formatting
	  plotArg <- as.matrix(plotInfo[,c("subtypeColors", "conf.colors")])
	  rownames(plotArg) <- plotInfo$Sample
	}
	
	pdf(paste(clustername,".pdf",sep=""))
	myHeatmap(out$testData,plotArg,file=paste(clustername,".cdt",sep=""),rowNames=rownames(out$testData), colNames = colnames(out$testData), bottomMar = 9)
	dev.off()
